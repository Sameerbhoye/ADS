...
statement: Given an array of task[] of size denoting the amount of work to be done for each task, the problem is to find the minimum amount work to be done on each day so that all the tasks can be completed at  in at most D days 
Note: On one day work can be done for only on task
